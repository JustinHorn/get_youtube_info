...
//         final id = '_HSylqgVYQI';
//         final scope = nock(id, 'rich-thumbnails', {
//           headers: {
...
//     test('Returns correct video metainfo', ()async {
//       final expected = await getFileAsMap('./test/files/videos/age-restricted/expected-info.json');
//       final id = 'LuZu9N53Vd0';
//       final scope = nock(id, 'age-restricted');
//       var info = await ytdl.getBasicInfo(id);
//       scope.done();
//       expect(info.formats.length, expected.formats.length);
...
//         final scope = nock(id, 'live-future');
//         var info = await ytdl.getBasicInfo(id);
//         scope.done();
//         expectOk(info);
//         expectOk(info.videoDetails);
...
//       final scope = nock(id, 'longest-upload');
//       var info = await ytdl.getBasicInfo(id);
//       scope.done();
//       expectOk(info);
//       expectOk(info.videoDetails);
...
//         final expected = await getFileAsMap('./test/files/videos/use-backups/expected-info.json');
//         final id = 'LuZu9N53Vd0';
//         final scope = nock(id, 'use-backups', {
...
//           player: false,
//         });
//         var info = await ytdl.getBasicInfo(id, { requestOptions: { maxRetries: 1 } });
//         scope1.done();
//         expectOk(info.formats.length);
//         expectOk(info.formats[0].url);
//       });
...
//       test('Uses backup endpoint', ()async {
//         final id = 'LuZu9N53Vd0';
//         final scope1 = nock(id, 'use-backups', {
//           watchHtml: false,
//           watchJson: [true, 200, '[]'],
//           embed: false,
//           player: false,
//         });
//         var info = await ytdl.getBasicInfo(id, { requestOptions: { maxRetries: 0 } });
//         scope1.done();
//         expectOk(info.formats.length);
//         expectOk(info.formats[0].url);
//       });
...
//         final scope = nock(id, 'non-existent');
...
//           await ytdl.getBasicInfo(id);
//         } catch (err) {
//           expectOk(/No video id found/.test(err.message));
//           return;
//         }
//         throw Error('should not get here');
//       });
//       test('Promise is rejected with caught error', done => {
//         final id = 'https://website.com';
//         ytdl.getBasicInfo(id).catch(err => {
//           expectOk(/Not a YouTube domain/.test(err.message));
//           done();
//         });
//       });
//     });
