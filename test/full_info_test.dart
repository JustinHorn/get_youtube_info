...
main() {
  ...
  group('ytdl.getInfo()', () {
    ...
    //   test('Retrieves video file', done => {
    //     final expected = require('./files/videos/regular/expected-info.json');
    //     final stream = ytdl.downloadFromInfo(expected);
    //     var scope;
    ...
    //     expectOk(info);
    //     expectOk(info.formats && info.formats.length);
    //     expectOk(info.formats.some(format => format.signatureCipher));
    //     info = await ytdl.getInfo(id);
    //     expectOk(info.formats.every(format => !format.signatureCipher));
    //     expectOk(info.formats.every(format => format.url));
    //     scope.done();
    //   });
    // });
